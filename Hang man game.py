...
time.sleep(2)
print("The objective of the game is to guess the encrypted word chosen by the computer.")
time.sleep(2)
...
while True:
    ...
    while True:
        if category.upper() == 'S':
            encryptedWord = random.choice(superHeroes)
            break
        elif category.upper() == 'F':
            encryptedWord = random.choice(fruits)
            break
        else:
    ...
    if playGame:
        encryptedWordList = list(encryptedWord)
        attempts = (len(encryptedWord) + 2)

        #Utility function to print User Guess List
        def printGuessedLetter():
            print("Your encrypted word is: " + ''.join(userGuesslist))


        #Adding blank lines to userGuesslist to create the blank secret word
        for n in encryptedWordList:
        ...
        while True:
            ...
            if letter in userGuesses:
                ...
                if letter in encryptedWordList:
                    print("Nice guess!")
                    if attempts > 0:
                        print("You have ", attempts, 'guess left!')
                    for i in range(len(encryptedWordList)):
                        if letter == encryptedWordList[i]:
                            letterIndex = i
            ...
            if joinedList.upper() == encryptedWord.upper():
                print("Yay! you won.")
                break
            elif attempts == 0:
                print("Too many Guesses!, Sorry better luck next time.")
                print("The encrypted word was: "+ encryptedWord.upper())
                break
